...
export class TimelineService {
  ...
  static async append(input: AppendEventInput): Promise<TimelineEvent> {
    ...
    log.start('append', correlationId, {
    ...
    try {
      ...
      log.succeed('append', correlationId, { id: created.id });
      return mapTimelineEvent(created);
    } catch (error) {
      if (error instanceof ServiceError) throw error;
      log.fail('append', correlationId, error);
  ...
  static async listForIncident(incidentId: string): Promise<TimelineEvent[]> {
    log.info('listForIncident', `inc-${incidentId}`, 'list');
